/*
  ...
  then use c as the hash value.  If you have a variable length array of 4-byte
  integers to hash, use hashword().  If you have a byte array (like a character
  string), use hashlittle().  If you have several byte arrays, or a mix of
  things, see the comments above hashlittle().
...
This is reversible, so any information in (a,b,c) before mix() isq still in
(a,b,c) after mix().
...
inline uint32_t hashlittle( const uint8_t* key,
                            size_t         length,
                            uint32_t       initval )
{
    ...
    if ( HASH_LITTLE_ENDIAN && ( (u.i & 0x3) == 0) )
    {
         ...
         * rest of the string.  Every machine with memory protection I've
         * seen does it on word boundaries, so is OK with this.  But VALGRIND
         * will still catch it and complain.  The masking trick does make the
         * hash noticably faster for short strings (like English words).
         */
 ...
 * with 2^^64 buckets, or if you want a second hash if you're not happy with
 * the first, or if you want a probably-unique 64-bit ID for the key.  *pc is
 * better mixed than *pb, so use *pc first.  If you want a 64-bit value do
 * something like "*pc + (((uint64_t)*pb)<<32)".
 */
inline void hashlittle2( 
    const uint8_t* key,       /* the key to hash */
    ...
    uint32_t*      pb )       /* IN: secondary initval, OUT: secondary hash */
{
    ...
    if ( HASH_LITTLE_ENDIAN && ((u.i & 0x3) == 0) )
    {
        ...
        case 2 : a+=k[0]&0xffff; break;
        case 1 : a+=k[0]&0xff; break;
        case 0 : *pc=c; *pb=b; return;  /* zero length strings require no
                                         * mixing */
        }
    }
    else if ( HASH_LITTLE_ENDIAN && ((u.i & 0x1) == 0) )
 ...
 */
inline uint32_t hashbig( const uint8_t *key, size_t length, uint32_t initval )
{
    uint32_t a,b,c;
    union { const void *ptr; size_t i; } u; /* to cast key to (size_t)
                                             * happily */
    ...
    if ( HASH_BIG_ENDIAN && ((u.i & 0x3) == 0) )
    {
        ...
        /*------------------------- handle the last (probably partial) block */
        /* 
         * "k[2]<<8" actually reads beyond the end of the string, but then
         * shifts out the part it's not allowed to read.  Because the string
         * is aligned, the illegal read is in the same word as the rest of
         * the string.  Every machine with memory protection I've seen does
         * it on word boundaries, so is OK with this.  But VALGRIND will
